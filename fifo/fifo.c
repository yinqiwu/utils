...
bool fifo_adds(fifo_t fifo, const void *item,size_t item_len){
  for (size_t i = 0; i < item_len;i++){
    if(fifo_add(fifo, (void *)(item + i)) == false){
		return false;
	}
  }
  return true;
...
bool fifo_gets(fifo_t fifo, void *item,size_t item_len){
  for (size_t i = 0; i < item_len; i++) {
    if(fifo_get(fifo, (void *)( item + i)) == false){
		return false;
	}
  }
  return true;
...
bool fifo_peek_offset(fifo_t fifo, void *item, size_t offset) {
  if (!fifo_is_empty(fifo)) {
    if (fifo->readoffset + offset < fifo->allocatedbytes) {
      memcpy(item, fifo->itemspace + fifo->readoffset + offset, fifo->itemsize);
    } else {
      memcpy(
          item,
          fifo->itemspace + (fifo->readoffset + offset - fifo->allocatedbytes),
          fifo->itemsize);
    }
    return true;
  } else {
