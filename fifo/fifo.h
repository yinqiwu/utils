...
extern "C" {
	...
	/**
	 * @brief Adds multiple item to the FIFO buffer
	 *
	 * This function writes an item to the fifo buffer back. This function
	 * affects the write pointer and the stored items counter.
	 *
	 * The number of bytes to be copied to the fifo buffer was defined when
	 * the fifo buffer was created with the function fifo_create() (size
	 * parameter).
	 *
	 * @param fifo Pointer to a fifo_descriptor structure.
	 * @param item Pointer to a location that holds data to be written to
	 * the fifo buffer.
	 *@param item_len The number of items that need to be added to the fifo
	 * @return  Returns true if there is space in the FIFO to add the item.
	 * If the buffer is full and no data can be copied it returns false.
	 */
	bool fifo_adds(fifo_t fifo, const void *item, size_t item_len);
	/**
	...
	/**
	 * @brief Obtains multiple item from the FIFO buffer.
	 *
	 * This function reads an item from the fifo buffer front. This function
	 * affects the read pointer and the stored items counter.
	 *
	 * The number of bytes to be copied to the provided buffer was defined
	 * when the fifo buffer was created with the function fifo_create()
	 * (size parameter).
	 *
	 * @param fifo Pointer to a fifo_descriptor structure.
	 * @param item Pointer to a location to hold the data read from the fifo
	 * buffer, this buffer should be sized appropiately to accomodate the
	 * data.
	 * @param item_len The number of items that need to be obtained from the
	 * fifo
	 *
	 * @return  Returns true if there is data available on the fifo buffer
	 * to be copied, if the buffer is empty and no data can be read this
	 * returns false.
	 */
	bool fifo_gets(fifo_t fifo, void *item, size_t item_len);

	/**
	 * @brief Get the data in the FIFO, but do not actually take out the
	 * data
	 *
	 *
	 * @param fifo Pointer to a fifo_descriptor structure.
	 ...
	 * returns false.
	 */
	bool fifo_peek_offset(fifo_t fifo, void *item, size_t offset);
	 ...
	 * @param fifo Pointer to a fifo_descriptor structure.
	 *
	 * @return This function returns true if the buffer is full, false
	 * otherwise.
	 */
	 ...
	 * @param fifo Pointer to a fifo_descriptor structure.
	 *
	 * @return This function returns true if the buffer is empty, false
	 * otherwise.
	 */
	 ...
	 *
	 * This function discards data from the back or the front side of the
	 * buffer, the side and the ammount of discarded data depends on the
	 * parameters passed.
	 *
	 * @param fifo Pointer to a fifo_descriptor structure.
	 * @param count The number of elements to discard from the buffer.
	 * @param side Defines if data should be discarted from the front or
	 * back side of the buffer.
	 *
